...
class SwiftstraintsTests: XCTestCase {
    ...
    objc func finish_testViewsAreNotRetained(
    
    func testViewsAreNotRetained() {
        let expect = expectation(description: "Let it collect")
        weak var tester: UIView?
        do {
            let superview = UIView()
            let leftView = UIView()
            superview.addSubview(leftView)
            let shorthandConstraints = NSLayoutConstraints("H:|-8-[\(leftView)(>=80,<=100)]-8-|")
//            shorthandConstraints.forEach { $0.isActive = true }
            tester = leftView
        }
        RunLoop.current.perform { 
            XCTAssert(tester == nil)
            expect.fulfill()
        }
    }
